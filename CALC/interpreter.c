...
void ftob(double a)
{
	...
	if (b > MAX || b < MIN)
	{
		printf("Sorry, out of range, please try \"print\"\n");
		return;
	}
	int neg_flag = 0;
	if (a < 0)
...
void printallf(void)
{
	...
	for (i = 0; i < 26; i++)
	{
		if (tag[i] == 1)
		{
			...
			if (sym[i] > MAX || sym[i] < MIN)
				printf(" Sorry, out of range for other form\n");
			else
			{
				printf(" "); ftoh(sym[i]);
				printf(" "); ftoo(sym[i]);
	...
	switch (p->type)
	{
		...
		switch (p->opr.oper)
		{
			...
			case MOD:		if ((ex(p->opr.op[0]) - (int)ex(p->opr.op[0]) != 0) ||
							...
							if (ex(p->opr.op[1]) == 0)
							{
								printf("\t\tError: The second operand can noy be zero\n");
								return 0;
							}
							else
